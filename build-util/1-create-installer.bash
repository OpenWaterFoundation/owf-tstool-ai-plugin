...
#     ./tstool/NN/plugins/owf-tstool-ai-plugin/
# - output is to the plugin 'dist' folder with filename like:
#     tstool-ai-plugin-1.0.0-win-2206060102.zip
...
createPluginZipFile() {
  ...
  "${sevenzip}" a -tzip ${zipFile} "owf-tstool-ai-plugin/${pluginVersion}"
  exitStatus=$?
...
now=$(date +%Y%m%d%H%M)
zipFile="${distFolder}/tstool-ai-plugin-${pluginVersion}-win-${now}.zip"
